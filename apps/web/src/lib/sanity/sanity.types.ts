...
// Query: *[_type == "event" && defined(dateTime) && (defined(slug.current))] | order(dateTime asc) [$start...$end] {    _id,    title,    description,    dateTime,    location,    registrationLink,    "slug": slug.current,    featureImage{      ...,      ...asset->{        "alt": coalesce(altText, originalFilename, "no-alt"),        "blurData": metadata.lqip,        "dominantColor": metadata.palette.dominant.background      }    }  }
